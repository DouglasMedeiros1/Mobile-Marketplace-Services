...
interface ServiceWithProposalCount extends Service {
  proposalCount?: number;
}

const ClienteHomeScreen: React.FC = () => {
  const navigation = useNavigation<HomeNavigationProp>();
  const {user} = useAuth();
  
  const [services, setServices] = useState<ServiceWithProposalCount[]>([]);
  const [allServices, setAllServices] = useState<ServiceWithProposalCount[]>([]);
  ...
  const loadServices = useCallback(async () => {
    try {
      ...
      // Filter only services created by the current user
      const myServices = data.filter(service => service.user_id === user?.id);
      
      // Load proposal counts for each service
      const servicesWithCounts = await Promise.all(
        myServices.map(async (service) => {
          try {
            const proposals = await proposalsService.getProposalsByService(service.id);
            return {...service, proposalCount: proposals.length};
          } catch {
            return {...service, proposalCount: 0};
          }
        })
      );
      
      setAllServices(servicesWithCounts);
  ...
  const renderServiceCard = ({item}: {item: ServiceWithProposalCount}) => (
    <TouchableOpacity
      style={styles.card}
      ...
      <View style={styles.cardDetails}>
        ...
        {item.proposalCount !== undefined && item.proposalCount > 0 && (
          <View style={styles.proposalBadge}>
            <Icon name="description" size={16} color="#2196F3" />
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>Nenhum serviço criado</Text>
